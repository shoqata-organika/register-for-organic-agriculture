import { AdmissionEntry } from '@/api/types/admission';
...
interface Props {
  entries: Array<AdmissionEntry>;
  crops: Array<Code>;
  onEdit: (entry: AdmissionEntry) => void;
}
function AdmissionEntryList({ entries, crops, onEdit }: Props) {
                ...
                {(entries || []).map((entry) => (
                  <tr key={entry.id || entry._tempId}>
                    <td className="whitespace-nowrap py-2 pl-4 pr-3 text-sm text-gray-500 sm:pl-0">
                      {getDisplayName(indexedCrops[entry.crop_id])}
                    </td>
                    <td className="whitespace-nowrap px-2 py-2 text-sm text-gray-500">
                      {getDisplayName(indexedPoC[entry.part_of_crop_id])}
                    </td>
                    <td className="whitespace-nowrap px-2 py-2 text-sm text-gray-500">
                      {t(entry.cropState)}
                    </td>
...
export default AdmissionEntryList;
